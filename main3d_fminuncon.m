...
%     chains(5).P = [0,0.4,0.5,1];
    ...
    for i = 1:size(chains,2)
        objfun = @(x)norm(chains(i).P - fk2(chains(i), x))^2 + penaltyfunc_fminuncon(chains(i),x);
        [chains(i).x0, fval] = fminunc(objfun,chains(i).x0);     
